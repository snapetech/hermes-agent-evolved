...
def _update_check_cache_seconds() -> int:
    try:
        return max(0, int(os.getenv("HERMES_UPDATE_CHECK_CACHE_SECONDS", str(6 * 3600))))
    except ValueError:
        return 6 * 3600


# Cache update check results for 6 hours by default to avoid repeated git fetches.
_UPDATE_CHECK_CACHE_SECONDS = _update_check_cache_seconds()
...
def check_for_updates() -> Optional[int]:
    ...
    try:
        if cache_file.exists():
            ...
            if cached.get("version") == _UPDATE_CHECK_CACHE_VERSION and now - cached.get("ts", 0) < _UPDATE_CHECK_CACHE_SECONDS:
    ...
    # Write cache
    try:
        cache_file.write_text(json.dumps({"version": _UPDATE_CHECK_CACHE_VERSION, "ts": now, "behind": behind}))
    except Exception:
        pass
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        if behind and behind > 0:
            ...
            noun, context = update_behind_words(behind)
            right_lines.append(
                f"[bold yellow]⚠ {behind} {noun} {context}[/]"
                f"[dim yellow] — {update_action_prefix()} [bold]{recommended_update_command()}[/bold] {update_action_suffix()}[/]"
            )
    except Exception:
