...
class TestCLILoadingIndicator:
    ...
    def test_reload_sets_busy_state_and_prints_status(self, capsys):
        cli_obj = self._make_cli()
        seen = {}
        ...
        with patch.object(cli_obj, "_reload_runtime_state", side_effect=fake_reload), \
             patch.object(cli_obj, "_invalidate") as invalidate_mock:
            assert cli_obj.process_command("/reload")

        output = capsys.readouterr().out
        assert "⏳ Reloading runtime state..." in output
        assert "runtime reloaded" in output
        assert seen == {
            "running": True,
            "status": "Reloading runtime state...",
        }
        assert cli_obj._command_running is False
        assert cli_obj._command_status == ""
        assert invalidate_mock.call_count == 2
