...
@pytest.mark.asyncio
async def test_status_command_includes_compaction_counts():
    session_entry = SessionEntry(
        session_key=build_session_key(_make_source()),
        session_id="sess-1",
        created_at=datetime.now(),
    ...
    with patch("agent.compaction_report.load_session_compaction_data", return_value={
        "compaction_metrics": {
            "event_count": 3,
    ...
    assert "**Compactions:** 3" in result
