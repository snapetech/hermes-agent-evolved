...
def _safe_profile_identifier(value: Any, fallback: str) -> str:
    raw = str(value or fallback).strip() or str(fallback)
    return re.sub(r"[^A-Za-z0-9_.-]+", "-", raw).strip("-") or str(fallback)
...
class ChannelProfile:
    """Per-channel gateway behavior overlay."""
    ...
    @classmethod
    def from_dict(cls, channel_id: str, data: Dict[str, Any]) -> "ChannelProfile":
        safe_name = _safe_profile_identifier(data.get("name"), str(channel_id))
        session_scope = str(data.get("session_scope") or "default").strip().lower()
        if session_scope not in {"default", "channel_shared"}:
            session_scope = "default"
        return cls(
            name=safe_name,
            session_scope=session_scope,
            db_namespace=_safe_profile_identifier(data.get("db_namespace"), safe_name),
            memory_namespace=_safe_profile_identifier(data.get("memory_namespace"), safe_name),
            model=dict(data.get("model") or {}) if isinstance(data.get("model"), dict) else {},
            fallback_providers=[
                dict(item)
                for item in (data.get("fallback_providers") or [])
                if isinstance(item, dict)
            ] if isinstance(data.get("fallback_providers"), list) else [],
            reasoning=dict(data.get("reasoning") or {}) if isinstance(data.get("reasoning"), dict) else {},
            response_style=(
                dict(data.get("response_style") or {})
                if isinstance(data.get("response_style"), dict)
                else {}
            ),
            tools=dict(data.get("tools") or {}) if isinstance(data.get("tools"), dict) else {},
            system_prompt_append=str(data.get("system_prompt_append") or ""),
        )
...
class GatewayConfig:
    ...
    # Per-platform channel profiles keyed by platform value then chat/channel ID.
    channel_profiles: Dict[Platform, Dict[str, ChannelProfile]] = field(default_factory=dict)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "channel_profiles": {
                p.value: {cid: profile.to_dict() for cid, profile in profiles.items()}
                for p, profiles in self.channel_profiles.items()
            },
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        channel_profiles: Dict[Platform, Dict[str, ChannelProfile]] = {}
        raw_profiles = data.get("channel_profiles", {})
        if isinstance(raw_profiles, dict):
            for platform_name, platform_profiles in raw_profiles.items():
                try:
                    platform = Platform(platform_name)
                except ValueError:
                    continue
                if not isinstance(platform_profiles, dict):
                    continue
                parsed: Dict[str, ChannelProfile] = {}
                for channel_id, profile_data in platform_profiles.items():
                    if isinstance(profile_data, dict):
                        parsed[str(channel_id)] = ChannelProfile.from_dict(
                            str(channel_id),
                            profile_data,
                        )
                if parsed:
                    channel_profiles[platform] = parsed
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if isinstance(gateway_section, dict):
                channel_profiles = gateway_section.get("channel_profiles")
            if channel_profiles is None:
                channel_profiles = yaml_cfg.get("channel_profiles")
            if isinstance(channel_profiles, dict):
                gw_data["channel_profiles"] = channel_profiles
