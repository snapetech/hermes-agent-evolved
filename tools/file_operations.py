...
from agent.file_safety import (
    ...
    get_write_block_error as _shared_get_write_block_error,
...
def _get_write_block_error(path: str) -> Optional[str]:
    """Return a specific denial reason for a blocked write path."""
    return _shared_get_write_block_error(path)
...
class FormatterResult:
    """Result from formatter-style validation."""
    success: bool = True
    skipped: bool = False
    output: str = ""
    message: str = ""
...
FORMATTERS = {
    '.py': 'black --check {file} 2>&1',
    '.js': 'npx prettier --check {file} 2>&1',
    '.ts': 'npx prettier --check {file} 2>&1',
    '.go': 'gofmt -d {file} 2>&1',
    '.rs': 'rustfmt --check {file} 2>&1',
}
...
class ShellFileOperations(FileOperations):
    ...
    def delete_file(self, path: str) -> WriteResult:
        ...
        if _is_write_denied(path):
            return WriteResult(error=_get_write_block_error(path) or f"Delete denied: {path} is a protected path")
    ...
    def move_file(self, src: str, dst: str) -> WriteResult:
        ...
        for p in (src, dst):
            if _is_write_denied(p):
                return WriteResult(error=_get_write_block_error(p) or f"Move denied: {p} is a protected path")
        result = self._exec(
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        if _is_write_denied(path):
            return WriteResult(error=_get_write_block_error(path) or f"Write denied: '{path}' is a protected system/credential file.")
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        # Block writes to sensitive paths
        if _is_write_denied(path):
            return PatchResult(error=_get_write_block_error(path) or f"Write denied: '{path}' is a protected system/credential file.")
        ...
        # Post-edit validation signals feed back into higher-level repair loops.
    ...
    def _check_formatter(self, path: str) -> FormatterResult:
        """
        Run formatter-style validation after editing.

        This intentionally does not rewrite files. It only reports whether the
        current file already matches the canonical formatter output.
        """
        ext = os.path.splitext(path)[1].lower()

        if ext not in FORMATTERS:
            return FormatterResult(skipped=True, message=f"No formatter check for {ext} files")

        formatter_cmd = FORMATTERS[ext]
        base_cmd = formatter_cmd.split()[0]

        if not self._has_command(base_cmd):
            return FormatterResult(skipped=True, message=f"{base_cmd} not available")

        cmd = formatter_cmd.replace("{file}", self._escape_shell_arg(path))
        result = self._exec(cmd, timeout=30)

        output = result.stdout.strip()
        if ext == '.go' and result.exit_code == 0 and output:
            # gofmt -d exits 0 even when the file would change, so treat any
            # diff output as a formatter mismatch.
            return FormatterResult(success=False, output=output)
    ...
    def _validate_file(self, path: str) -> Dict[str, Any]:
        """
        Collect post-edit validation signals for repair loops and routing.
        """
        lint_result = self._check_lint(path)
        formatter_result = self._check_formatter(path)
