...
:::warning Token types
The Copilot API endpoint used by Hermes does **not** support Personal Access Tokens (`ghp_*` or `github_pat_*`). Supported token types:
...
If your `gh auth token` returns a PAT, use `hermes model` to authenticate via OAuth instead.
:::
