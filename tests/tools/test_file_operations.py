...
class TestFormatterResult:
    def test_skipped(self):
        r = FormatterResult(skipped=True, message="No formatter check for .md files")
        d = r.to_dict()
        assert d["status"] == "skipped"
        assert d["message"] == "No formatter check for .md files"

    def test_success(self):
        r = FormatterResult(success=True, output="")
        d = r.to_dict()
        assert d["status"] == "ok"

    def test_error(self):
        r = FormatterResult(success=False, output="would reformat x.py")
        d = r.to_dict()
        assert d["status"] == "error"
        assert "reformat" in d["output"]
