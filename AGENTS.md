...
When merging `upstream/main` from `NousResearch/hermes-agent`, upstream
supersedes local fork implementation. Resolve conflicts by taking upstream
first, then back-build any Snapetech deployment gaps as explicit overlay
changes on top of the official code. Do not preserve fork code merely because
it worked before the sync; if upstream now owns that surface, align with
upstream and repair local behavior afterward.
