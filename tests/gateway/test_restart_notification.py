...
@pytest.mark.asyncio
async def test_restart_idle_command_still_writes_notify_file(tmp_path, monkeypatch):
    """The deferred restart path should still notify the requester after restart."""
    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)

    runner, _adapter = make_restart_runner()
    runner.request_restart = MagicMock(return_value=True)
    runner._running_turns["session-1"] = {"started_at": 0}

    source = make_restart_source(chat_id="77")
    event = MessageEvent(
        text="/restart idle",
        message_type=MessageType.TEXT,
        source=source,
        message_id="m-idle-notify",
    )
