...
| `COPILOT_GITHUB_TOKEN` | GitHub token for Copilot API — first priority (OAuth `gho_*`; PATs `ghp_*` / `github_pat_*` are **not supported**) |
...
| `EMAIL_PASSWORD` | Password, app password, Proton Bridge password, or SMTP token for the email account |
| `EMAIL_IMAP_HOST` | IMAP hostname or Proton Bridge listener for the email adapter |
| `EMAIL_IMAP_PORT` | IMAP port |
| `EMAIL_IMAP_SECURITY` | IMAP transport security: `ssl`, `starttls`, or `plain` |
