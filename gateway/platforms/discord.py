...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _consume_bot_task_result(self, task: asyncio.Task) -> None:
        """Retrieve background client task failures so asyncio does not emit them."""
        try:
            task.result()
        except asyncio.CancelledError:
            return
        except Exception as e:  # pragma: no cover - defensive logging
            level = logging.WARNING if self._running else logging.DEBUG
            logger.log(level, "[%s] Discord client task ended: %s", self.name, e, exc_info=True)

    async def _cleanup_failed_connect(self) -> None:
        """Best-effort cleanup after connect() fails before the bot is ready."""
        if self._client:
            try:
                await self._client.close()
            except Exception as e:  # pragma: no cover - defensive logging
                logger.debug("[%s] Error closing Discord client after failed connect: %s", self.name, e)
        ...
        if self._bot_task and not self._bot_task.done():
            ...
            try:
                ...
                logger.debug("[%s] Discord client task failed during cleanup: %s", self.name, e)
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._bot_task.add_done_callback(self._consume_bot_task_result)
    ...
    async def disconnect(self) -> None:
        ...
        if self._bot_task and not self._bot_task.done():
            ...
            try:
                ...
                logger.debug("[%s] Discord client task failed during disconnect: %s", self.name, e)
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        # accepted text-channel message so each conversation is isolated (like Slack).
        # Messages already inside threads or DMs are unaffected.
        # no_thread_channels: channels where bot responds directly without thread.
        auto_threaded_channel = None
        if not is_thread and not isinstance(message.channel, discord.DMChannel):
            no_thread_channels_raw = self.config.extra.get("no_thread_channels")
            if no_thread_channels_raw is None:
                no_thread_channels_raw = os.getenv("DISCORD_NO_THREAD_CHANNELS", "")
            if isinstance(no_thread_channels_raw, list):
                no_thread_channels = {
                    str(ch).strip() for ch in no_thread_channels_raw if str(ch).strip()
                }
            else:
                no_thread_channels = {
                    ch.strip()
                    for ch in str(no_thread_channels_raw).split(",")
                    if ch.strip()
                }
